...
private:
    ...
    std::string interestedLabelPath;
...
public:
    rknnPool(const std::string modelPath, int threadNum, const std::string& labelPath, const std::string& interestedLabelPath, float nmsThresh, float boxConfThresh);
    int init();
...
template <typename rknnModel, typename inputType, typename outputType>
rknnPool<rknnModel, inputType, outputType>::rknnPool(const std::string modelPath, int threadNum, const std::string& labelPath, const std::string& interestedLabelPath, float nmsThresh, float boxConfThresh)
{
    this->modelPath = modelPath;
    this->threadNum = threadNum;
    this->id = 0;
    this->nmsThreshold = nmsThresh;
    this->boxConfThreshold = boxConfThresh;
    this->labelPath = labelPath;
    this->interestedLabelPath = interestedLabelPath;
}
...
int rknnPool<rknnModel, inputType, outputType>::init()
{
    try
    {
        ...
        for (int i = 0; i < this->threadNum; i++)
            models.push_back(std::make_shared<rknnModel>(this->modelPath.c_str(), this->labelPath, this->interestedLabelPath, this->nmsThreshold, this->boxConfThreshold));
    }
